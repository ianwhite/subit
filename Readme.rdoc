...
== Example
  ...
  subit = Subit.define do |p|
    ...
    h.rule "init", "isn't it", :original => "isn't it"
    ...
    p.define :html do |h|
      ...
      h.rule /Product\((\w+)\)/,
        ...
        :original => lambda {|name| "Product(#{Product.find_by_name(name).id}-#{name})"}
    end
  ...
  # to parse and replace the original content with any original rules:
  subit.parse_with_parse_original!(content)
